...
@Table({
  ...
  indexes: [
    ...
    // with args for a wallet at a given block height.
    {
      unique: true,
      fields: ['blockHeight', 'walletAddress', 'formula', 'args'],
    },
    {
      // Speeds up queries. Composite indexes are most efficient when equality
      // tests are first and ranges second.
      fields: ['walletAddress', 'blockHeight'],
    },
    {
...
export class WalletComputation extends Model {
  @AllowNull(false)
  @Column
  ...
  ): Promise<WalletComputation[]> {
    return await WalletComputation.bulkCreate(
